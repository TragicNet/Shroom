...
if (!class_exists('VG_Admin_To_Frontend')) {
	...
	class VG_Admin_To_Frontend {
			...
			<script>
				jQuery(window).on('load', function () {
					if (jQuery('html').hasClass('vgca-only-admin-content') && window.location.href.indexOf('vgfa_internal=1') > -1) {
						jQuery(<?php echo json_encode(sanitize_text_field($back_button_container)); ?>).prepend('<button type="button" onclick="window.history.back(2); event.preventDefault; return false;" class="page-title-action"><span class="dashicons dashicons-arrow-left-alt2" style="height: auto"></span> <?php echo sanitize_text_field(__('Go back', VG_Admin_To_Frontend::$textname)); ?></button><br>');
					}
				});
			</script>
			<?php
		...
		function prepare_loose_url($url) {
			...
			$url = remove_query_arg(array('post', 'token', '_wpnonce', 'user_id', 'wp_http_referer', 's', 'page_id', 'revision', 'tag_ID'), $url);
