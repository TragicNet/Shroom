...
if (!class_exists('WPFA_Global_Dashboard')) {
	...
	class WPFA_Global_Dashboard {
		...
		function set_site_from_url__premium_only() {
			if (is_multisite() && !empty($_GET['wpfa_site_id']) && !empty($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'wpfa')) {
				$site_id = (int) $_GET['wpfa_site_id'];
				$this->switch_site($site_id);
				wp_redirect(remove_query_arg(array('wpfa_site_id', '_wpnonce')));
				exit();
			}
		}
		...
		function get_dashboard_url($blog_id) {
			$url = add_query_arg('wpfa_site_id', $blog_id, VG_Admin_To_Frontend_Obj()->get_settings('redirect_to_frontend', get_site_url($this->get_dashboard_site_id(), '/')));
		...
		function get_site_selector__premium_only($atts = array(), $content = '') {
			...
			ob_start();
			include VG_Admin_To_Frontend::$dir . '/views/frontend/site-selector.php';
			return ob_get_clean();
		}
