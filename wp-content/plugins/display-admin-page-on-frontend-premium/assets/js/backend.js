...
var wpFrontendAdminBackend = {
	...
	openFrontendLinksInMainWindow: function () {
		jQuery('body').on('click', 'a', function (e) {
			...
			if (typeof url !== 'string' || url.indexOf('http') < 0 || url.indexOf('wpfaNoInterfere') > -1 || !window.wpfaIframeId || jQuery(this).parents('.instant-img-container').length || url.indexOf('&preview=true') > -1) {
				return true;
			}
	...
	addRefererToUrl: function (url, rawParentUrl) {
		if (url.indexOf('wpfa_referrer') > -1) {
			return url;
		}
		try {
			// Add a parameter to frontend URLs to indicate where they came from
